...
use std::mem;
use addr::{Address, Integer, Masked};
use cpu::CPU;
use mem::Addressable;
...
bitflags! {
    ...
    flags StatusFlags: u8 {
        const CarryFlag             = 1 << 0,   // C
        const ZeroFlag              = 1 << 1,   // Z
        const InterruptDisableFlag  = 1 << 2,   // I
        const DecimalFlag           = 1 << 3,   // D
        const BreakFlag             = 1 << 4,   // B
        const UnusedAlwaysOnFlag    = 1 << 5,   // -
        const OverflowFlag          = 1 << 6,   // V
        const NegativeFlag          = 1 << 7,   // N
    }
}

impl StatusFlags {
    fn set (&mut self, flags: StatusFlags, state: bool) {
        if state {
            self.insert(flags)
        } else {
            self.remove(flags)
...
impl<M: Addressable> Mos6502<M> {
    ...
    fn set_zn (&mut self, value: u8) -> u8 {
        self.sr.set(ZeroFlag, value == 0);
        self.sr.set(NegativeFlag, (value as i8) < 0);
        value
    }

    /// Push a value onto the stack
    fn push<T: Integer> (&mut self, value: T) {
...
impl<M: Addressable> CPU for Mos6502<M> {
    ...
    fn step (&mut self) -> usize {
        ...
        if self.reset {
            ...
            self.sr.insert(InterruptDisableFlag);
        ...
        if self.nmi {
            ...
            // See also http://6502.org/tutorials/interrupts.html
            let pc = self.pc; self.push(pc);
            let sr = self.sr.bits; self.push(sr);
        ...
        if self.irq && !self.sr.contains(InterruptDisableFlag) {
...
mod tests {
    ...
    fn initial_state () {
        ...
        assert_eq!(cpu.sr, UnusedAlwaysOnFlag);
        assert!(cpu.reset);
    }
    ...
    fn fetch_memory_contents_and_advance_pc () {
        let mut cpu = Mos6502::new(TestMemory);
        cpu.pc = 0x0012;
        let value: u8 = cpu.next(); assert_eq!(value, 0x12);
        let value: u8 = cpu.next(); assert_eq!(value, 0x13);
        let value: u16 = cpu.next(); assert_eq!(value, 0x1514);
        let value: u16 = cpu.next(); assert_eq!(value, 0x1716);
    }
    ...
    fn status_flags () {
        let mut cpu = Mos6502::new(TestMemory);
        cpu.sr = ZeroFlag | DecimalFlag | UnusedAlwaysOnFlag | NegativeFlag;
        cpu.sr.set(CarryFlag, true);
        cpu.sr.set(ZeroFlag, false);
        cpu.sr.set(OverflowFlag, true);
        cpu.sr.set(NegativeFlag, false);
        assert_eq!(cpu.sr, CarryFlag | DecimalFlag | UnusedAlwaysOnFlag | OverflowFlag);
    }
    ...
    fn zero_and_negative_values () {
        let mut cpu = Mos6502::new(TestMemory);
        cpu.set_zn(0);
        assert!(cpu.sr.contains(ZeroFlag));
        assert!(!cpu.sr.contains(NegativeFlag));
        cpu.set_zn(42);
        assert!(!cpu.sr.contains(ZeroFlag));
        assert!(!cpu.sr.contains(NegativeFlag));
        cpu.set_zn(142);
    ...
    fn state_after_nmi () {
        ...
        cpu.sr = CarryFlag | ZeroFlag | UnusedAlwaysOnFlag;
        cpu.sp = 0xff;
        ...
        assert_eq!(cpu.sr, CarryFlag | ZeroFlag | UnusedAlwaysOnFlag);
        assert_eq!(cpu.sp, 0xfc);
    }
    ...
    fn state_after_irq () {
        ...
        cpu.step();
        assert_eq!(cpu.pc, 0x1234);
        assert_eq!(cpu.sr, CarryFlag | ZeroFlag | InterruptDisableFlag | UnusedAlwaysOnFlag);
        assert_eq!(cpu.sp, 0xfc);
    }
    ...
    fn brk_bug () {
        ...
        assert!(!cpu.sr.contains(BreakFlag));
